...
int main()
{
	HANDLE hStdoutold, hStdOutNew; // дескрипторы буфера экрана DWORD dwWritten;  
	DWORD ddWritten;// для количества выведенных символов 
	...
	if (!WriteConsole(
		...
		&ddWritten, // количество выведенных символов 
		NULL)) // не используется 
